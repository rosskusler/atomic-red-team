...
## [Description from ATT&CK](https://attack.mitre.org/wiki/Technique/T1546.004)
...
## Atomic Test #3 - Add command to /etc/bashrc
Adds a command to the /etc/bashrc
...
| Name | Description | Type | Default Value | 
|------|-------------|------|---------------|
| command_to_add | Command to add to the /etc/bashrc file | string | /path/to/script.py|
...
```sh
echo "#{command_to_add}" >> /etc/bashrc
```
